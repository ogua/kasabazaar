<x-layouts.app>
    <div class="relative bg-red-50 py-10">
        ...
        <div class="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-lg border-2 border-royalblue-600">
            ...
            <!-- purchased items -->
            @if (count($shipping->puchaseditems) > 0)


                <div class="mb-6">
                    <div class="space-y-6">
                        <div class="p-6 border-2 rounded-lg" style="background-color: #4169E1;">
                            <p class="text-base text-white font-bold">Purchased Items
                                <span></span>
                            </p>
                                ...
                                <table class="w-full mt-4 text-left border-2 border-white">
                                    <thead>
                                        <tr class="bg-red-500 text-white">
                                            ...
                                            <th class="p-3 border text-base font-bold">Cost</th>

                                        </tr>
                                    </thead>
                                    <tbody>
                                        @php $totalQuantity = 0; @endphp
                                        @foreach ($shipping->puchaseditems as $item)
                        ...
                        <div class="p-6 border-2 rounded-lg" style="background-color: #4169E1;">
                            ...
                            @if ($receiver->mcountry?->name)
                                <p class="text-base text-white font-bold">Address:
                                    <span>{{ $receiver->mcountry?->name }}, {{ $receiver->mstate?->name }},
                                        {{ $receiver->mcity?->name }}</span>
                                </p>
                            @endif
                            ...
                            @if ($receiver->receiver_id_type)
                                <p class="text-base text-white font-bold">ID:
                                    <span>{{ $receiver->receiver_id_type }} :
                                        {{ $receiver->receiver_id_number }}</span>
                                </p>
                            @endif
            ...
            <div class="mt-6 flex justify-end">
                <div class="text-right">
                    @if (count($shipping->puchaseditems) > 0)
                        <p class="text-base text-gray-800 font-bold">Total Purchased Items:
                            <span
                                class="text-royalblue-700">${{ number_format($shipping->puchaseditems->sum('item_cost'), 2) }}</span>
                        </p>
                    @endif
                    <p class="text-base text-gray-800 font-bold">Shipping Cost:
                        <span class="text-royalblue-700">${{ number_format($shipping->shipping_cost, 2) }}</span>
                    </p>
