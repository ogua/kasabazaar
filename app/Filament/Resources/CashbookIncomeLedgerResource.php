...
use App\Filament\Resources\CashbookIncomeLedgerResource\Pages;
...
class CashbookIncomeLedgerResource extends Resource
{
                ...
                Forms\Components\Section::make('Ledger Entry')
                    ->schema([
                        ...
                        Forms\Components\Select::make('ledger_type')
                            ->label('Ledger Type')
                            ->options(IncomeLedgerType::class)
                            ->required(),
                        Forms\Components\TextInput::make('particulars')->default('Cash Book Receipt'),
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('ledger_type')
                    ->label('Ledger')
                    ->badge()
                    ->formatStateUsing(fn (IncomeLedgerType $state) => $state->getLabel())
                    ->color(fn (IncomeLedgerType $state) => $state->getColor()),
            ...
            ->filters([
                ...
                Tables\Filters\SelectFilter::make('year')
                    ->options(fn () => CashbookIncomeLedger::query()
