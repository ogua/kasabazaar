...
class CustomerLookupController extends CustomerBaseController
{
    ...
    public function previousReceivers(): JsonResponse
    {
        ...
        // Receivers from approved shipments
        $fromShipments = Receiver::whereHas('shipment', fn ($q) => $q->where('client_id', $clientId))
        ...
        // Receivers from submitted shipment requests (requests not yet approved)
        $fromRequests = ShipmentRequest::where('client_id', $clientId)
            ->whereNotNull('receivers')
            ->pluck('receivers')
            ->flatten(1)
            ->map(fn ($r) => [
            ...
            ->filter(fn ($r) => ! empty($r['receiver_name']));

        $rows = Collection::make([...$fromShipments, ...$fromRequests])
